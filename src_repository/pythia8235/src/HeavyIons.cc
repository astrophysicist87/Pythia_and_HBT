...
std::cout << "Transverse position: "
			...
			<< shift.px() << "   "
			<< shift.py() << "   ";
    ei.event[i].xProd(ei.event[i].xProd() + ChristopherPlumbergUnits::millimeter*shift.px());
    ei.event[i].yProd(ei.event[i].yProd() + ChristopherPlumbergUnits::millimeter*shift.py());
...
//std::cout << "Check2: sum these = " << ei.event.size() << endl;
...
bool Angantyr::next() {
  ...
  while ( itry-- ) {
   ...
   // Collect secondary elastic sub-collisions.
	...
	// Chris Plumberg
	const double cphi = std::cos(hiinfo.phi());
	const double sphi = std::sin(hiinfo.phi());

	for (auto const& subevent : subevents)
	for ( int i = 0, N = subevent.event.size(); i < N; ++i )
		std::cout << "Rotated transverse position: "
					<< scientific
					<< setprecision(8)
					<< hiinfo.phi() << "   "
					<< subevent.event[i].xProd()*1.e12 << "   "
					<< subevent.event[i].yProd()*1.e12 << "   "
					<< (  cphi * subevent.event[i].xProd() + sphi * subevent.event[i].yProd() )*1.e12 << "   "
					<< ( -sphi * subevent.event[i].xProd() + cphi * subevent.event[i].yProd() )*1.e12 << endl;
	// Chris Plumberg
