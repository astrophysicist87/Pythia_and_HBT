...
int main(int argc, char *argv[])
{
	...
	for (int iThread = 0; iThread < omp_get_max_threads(); iThread++)
	{
		...
		bool omit_longlived_decays = true;
...
if ( 1.e12*p.vProd().mCalc() < -50.0 and p.p().pT() < 0.2 and p.pz()*p.pz() < 1.0 and not particle_is_decay )
{
	cout << "START EVENT LISTING: " << iEvent << "   " << 1.e12*p.vProd().mCalc() << endl;
	pythiaVector[iThread].event.list(true, true, 8);
	cout << "END EVENT LISTING" << endl;
}
