...
vector<Vec4> unshifted_momenta, shifted_momenta;
  ...
  for (int iSpecies = 0; iSpecies < 9; ++iSpecies) {
    ...
    for (int i = 0; i < event.size(); ++i)
      if ( event[i].id() == idNow && event[i].isFinal() ) {
        ...
        pHadSum += event[i].p();
		unshifted_momenta.push_back( event[i].p() );
      }
...
///*
  ...
  // Now compute shifts in local rest frame
  for (int iSpecies = 0; iSpecies < 9; ++iSpecies) {
		...
		// Properties of current hadron species.
		//int idNow = IDHADRON[ iSpecies ];
		int iTab = ITABLE[ iSpecies ];    // Loop through pairs of identical particles and find shifts.
	//*/
...
cout << "Warning in BoseEinstein::shiftEvent: "
	...
	for (int i = 0; i < nStored[9]; ++i)
	{
		...
		hadronBE[i].p.bst( pHadSum );
	}

	// Check 4-momentum conservation
	pHadSumFinal = Vec4(0.0, 0.0, 0.0, 0.0);
	for (int i = 0; i < nStored[9]; ++i)
	{
		pHadSumFinal += hadronBE[i].p;
