...
void RopeDipole::propagateInit(double deltat) {
  ...
  Vec4 newv1 = Vec4(d1.getParticlePtr()->xProd() + FM2MM * deltat * pcm.px() / mTc,
                d1.getParticlePtr()->yProd() + FM2MM * deltat * pcm.py() / mTc, 0, 0);
  Vec4 newv2 = Vec4(d2.getParticlePtr()->xProd() + FM2MM * deltat * pam.px() / mTa,
                d2.getParticlePtr()->yProd() + FM2MM * deltat * pam.py() / mTa, 0, 0);
  ///===CJP(end)===
...
void RopeDipole::propagate(double deltat, double m0) {
  ...
  for (map<double, Particle*>::iterator eItr = excitations.begin();
    ...
    ///===CJP(begin)===
    if (em.pT() > 0.0){
      Vec4 newVert = Vec4(eItr->second->xProd() + FM2MM * deltat * em.px() / em.pT(),
                eItr->second->yProd() + FM2MM * deltat * em.py() / em.pT(), 0, 0);
