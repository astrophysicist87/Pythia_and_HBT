...
double BoseEinstein::get_1D_source_size(int iSpecies)
{
	...
	else
	{
		...
		for (int i2 = i1 + 1;            i2 < nStored[iSpecies+1];   ++i2)
		{
			...
			double xDiffval      = xDiff.pAbs() * MM2FM;	// fm
			if (xDiffval < 1e-6) continue;					// closer than 1e-6 fm or negative
			xDiffval            /= HBARC;					// 1/GeV
...
bool BoseEinstein::shiftEvent( Event& event )
{
	...
	else
	{
		...
		Vec4 pSumOriginal, pSumShifted, pDiffByComp;
		double pxAbsSumOriginal = 0.0;
		double pyAbsSumOriginal = 0.0;
		double pzAbsSumOriginal = 0.0;
		vector<double> pxAbsOriginals;
		vector<double> pyAbsOriginals;
		...
		//cout << "New method (0): pDiffByTrans = " << pDiffByTrans;
...
void BoseEinstein::shiftPair_fixedQRef( int i1, int i2, int iTab )
{
  ...
  // check that computed factor was correct solution
  {
	...
	local_p1.e( sqrt( local_p1.pAbs2() + m1_2 ) );
	local_p2.e( sqrt( local_p2.pAbs2() + m2_2 ) );
			...
			)
{
	...
	if ( include_phase_space )
		...
		{
			//if ( BE_VERBOSE and pairCount % 1000 == 0 )
			//	std::cout << "set_LHS(): pairCount == " << pairCount << std::endl;
	...
	if ( BE_VERBOSE )
		std::cout << "set_LHS(): Loop completed." << std::endl;
			...
			)
{
	...
	if ( BE_VERBOSE )
		std::cout << "Setting sorted xDiffs..." << std::endl;
					...
					vector<double> & pairCompensationShifts, int iTab)
{
	...
	vector<double> denBar;
	if ( BE_VERBOSE )
		std::cout << "Setting pair density..." << std::endl;
	...
	if ( BE_VERBOSE )
		std::cout << "Evaluating shift relation..." << std::endl;
			...
			vector< pair< double, pair <int,int> > > & sortedPairs, int iTab )
{
	...
	if ( BE_VERBOSE )
		std::cout << "Computing effective source..." << std::flush;
	...
	for (int iPair = 1; iPair < (int)sortedPairs.size()-1; iPair++)
	{
		...
		if ( BE_VERBOSE and iPair % 1000000 == 0 )
			std::cout << "Set_effective_source(): iPair == " << iPair << std::endl;
