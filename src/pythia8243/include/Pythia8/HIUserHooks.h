...
public:
  ...
  /// Constructor.
  HIInfo()
    : idProjSave(0), idTargSave(0), bSave(0.0), phiSave(0.0), NSave(0), NAccSave(0),
      sigmaTotSave(0.0), sigmaNDSave(0.0), sigErr2TotSave(0.0),
      sigErr2NDSave(0.0), weightSave(0.0), weightSumSave(0.0),
      sigmaTotThisEventSave(0.0), sigmaNDThisEventSave(0.0),
  ...
  double b() const {
   ...
   ///===CJP(begin)===
   /// The impact-parameter direction in the current event.
   double phi() const {
     return phiSave;
   }
   ///===CJP(begin)===
...
private:
  ...
  void addAttempt(double T, double bin, double bphiin, double bweight);
