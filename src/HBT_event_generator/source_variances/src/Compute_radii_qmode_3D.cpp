...
void SourceVariances::Average_source_moments()
{
	...
	for (int iKL = 0; iKL < n_KL_bins; ++iKL)
	{
		...
		x_S[index3D]	 /= (norm+local_eps);
		x2_S[index3D]	 /= (norm+local_eps);
		y_S[index3D]	 /= (norm+local_eps);
		y2_S[index3D]	 /= (norm+local_eps);
		z_S[index3D]	 /= (norm+local_eps);
		z2_S[index3D]	 /= (norm+local_eps);
		xo_S[index3D] 	 /= (norm+local_eps);
		xo2_S[index3D] 	 /= (norm+local_eps);
		xs_S[index3D] 	 /= (norm+local_eps);
		xs2_S[index3D] 	 /= (norm+local_eps);
		xl_S[index3D] 	 /= (norm+local_eps);
		xl2_S[index3D] 	 /= (norm+local_eps);
		t_S[index3D] 	 /= (norm+local_eps);
		t2_S[index3D] 	 /= (norm+local_eps);
		x_t_S[index3D] 	 /= (norm+local_eps);
		y_t_S[index3D] 	 /= (norm+local_eps);
		z_t_S[index3D] 	 /= (norm+local_eps);
		x_y_S[index3D] 	 /= (norm+local_eps);
		x_z_S[index3D] 	 /= (norm+local_eps);
		y_z_S[index3D] 	 /= (norm+local_eps);
		xo_t_S[index3D]  /= (norm+local_eps);
		xs_t_S[index3D]  /= (norm+local_eps);
		xl_t_S[index3D]  /= (norm+local_eps);
		xo_xs_S[index3D] /= (norm+local_eps);
		xo_xl_S[index3D] /= (norm+local_eps);
		xs_xl_S[index3D] /= (norm+local_eps);
