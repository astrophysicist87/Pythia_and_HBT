...
void read_testIOGenEvent(std::ostream & os)
{
    ...
    const char infile[] = "/scratch/blixen/plumberg/Pythia_and_HBT/src/HepMC/HepMC-2.06.09/test/testIOGenEvent.input";
...
void read_variousFormats(std::ostream & os)
{
    ...
    const char infile[] = "/scratch/blixen/plumberg/Pythia_and_HBT/src/HepMC/HepMC-2.06.09/test/testHepMCVarious.input";
    std::ifstream is( infile );
    if( !is ) {
      std::cerr << "cannot open " << infile << std::endl;
      exit(-1);
