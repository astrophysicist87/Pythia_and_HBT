...
source $SCRIPTS_DIRECTORY/slurm_env.sh
...
do
	...
	executableString="./driver_slurm.sh NDATASETS=$NDATASETS chosen_QUEUENAME=$chosen_QUEUENAME chosen_Pythia_walltime=$chosen_Pythia_walltime chosen_HBT_walltime_per_event_class=$chosen_HBT_walltime_per_event_class ${specs[i]} &> driver_slurm.out"
	generate_sh $executableString > $HOME_RESULTS_DIRECTORY/job-${job}/submit.sh

	cp $SCRIPTS_DIRECTORY/driver_slurm.sh           $HOME_RESULTS_DIRECTORY/job-${job}
	#cp $SCRIPTS_DIRECTORY/run_Pythia.sh           $HOME_RESULTS_DIRECTORY/job-${job}
	cp $SCRIPTS_DIRECTORY/run_Pythia.slurm          $HOME_RESULTS_DIRECTORY/job-${job}
	cp $SCRIPTS_DIRECTORY/run_HBT_analysis_slurm.sh    $HOME_RESULTS_DIRECTORY/job-${job}
	cp $SCRIPTS_DIRECTORY/post_process_Pythia_slurm.sh $HOME_RESULTS_DIRECTORY/job-${job}
	cp $SCRIPTS_DIRECTORY/rerun_slurm.sh            $HOME_RESULTS_DIRECTORY/job-${job}
	cp $SCRIPTS_DIRECTORY/defaults.sh \
       $SCRIPTS_DIRECTORY/specs.sh    \
       $SCRIPTS_DIRECTORY/env.sh      \
       $SCRIPTS_DIRECTORY/omp_env.sh  \
       $SCRIPTS_DIRECTORY/slurm_env.sh              $HOME_RESULTS_DIRECTORY/job-${job}/scripts
done
